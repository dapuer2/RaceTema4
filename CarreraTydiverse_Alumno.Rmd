...
# Seleccionar las variables ip_id, package y country sin almacenar el resultado
 cran %>%
...
```{r}
cran %>%
  filter(country == "ES" & as.numeric(substring(r_version, 1, 3)) > 3.4)
```
...
```{r}
cran %>%
  filter(size > 100500 & grepl("linux-gnu", r_os))
```
...
cran %>%
  group_by(country) %>%
  summarise(traffic = sum(size)) %>%
  mutate(cumulative_traffic = cumsum(traffic),
         total_traffic = sum(traffic),
         threshold = 0.9 * total_traffic) %>%
  filter(cumulative_traffic >= threshold) %>%
  arrange(desc(traffic)) %>%
  ggplot(aes(x = country, y = traffic)) +
  geom_point() +
  labs(x = "Código de país", y = "Tráfico generado")
